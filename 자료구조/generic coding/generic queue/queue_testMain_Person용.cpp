...
void input(Queue *qPtr)
{
	...
	char temp[100];
	while (1) {
		printf("enqueue할 이름 입력(""end"" 입력 시 종료) : ");
		scanf("%s", temp);
...
void myDelete(Queue *qPtr)
{
	...
	personClear(&dequeData); // 메모리 누수 방지를 위해 호출
}
/*------------------------------------------------------------------------------------------------------------
