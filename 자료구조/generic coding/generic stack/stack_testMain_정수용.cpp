...
int main()
{
	...
	createStack(&stk, 5, sizeof(int)); /* 스택 초기화*/
	...
	destroyStack(&stk, sizeof(int), intClear);
	return 0;
}
/*--------------------------------------------------------------------------------------
...
void mInput(Stack *sp)
{
	...
	while (1) {
		...
		if (push(sp, &data, sizeof(int), intMemCopy) == FALSE)
			printf("push 실패!\n");
	}
}
/*--------------------------------------------------------------------------------------
...
void myDelete(Stack *sp)
{
	...
	scanf("%d", &cnt);
	for (i = 0; i<cnt; i++) {
		res = pop(sp, &popData, sizeof(int), intMemCopy, intClear);
...
{
	printStack(sp, sizeof(int), intPrint);
}
/*--------------------------------------------------------------------------------------
